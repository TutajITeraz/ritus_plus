...
// Row helpers live in utils/tableRows.js so the batch "Process Table" runner
// on the projects list can apply the same Auto Propagate as this table.
...
const DataTable = ({ tableStructure, data = [], setData, structureKey = "content" }) => {
  ...
  const sequenceKey = sequenceKeyOf(tableStructure);
  ...
  const handleMerge = useCallback(() => {
    ...
    const baseRowId = mergedRows[0]?._internalId;
    const newRow = mergeRowsData(mergedRows, tableStructure, sequenceKey);
  ...
  const handleAutoPropagate = useCallback(() => {
    if (!supportsAutoPropagate(tableStructure)) {
      toaster.create({
        title: "Error",
        description:
          "Auto Propagate is only supported for structures with function/rite/subrite columns",
        type: "error",
        duration: 3000,
      });
      return;
    ...
    const updatedData = autoPropagateRows(data, tableStructure, sequenceKey);
    validateUniqueIds(updatedData);
    setData(updatedData);
    setSelectedRows(new Set());
    setValidationErrors([]);
    ...
    toaster.create({
      title: "Success",
      description: "Auto propagate completed",
      type: "success",
      duration: 3000,
    });
  }, [data, tableStructure, sequenceKey, validateTable, validateUniqueIds]);
  ...
  return (
    <VStack width="full" gap="5" alignItems="start">
      <HStack mb={2}>
        ...
        <Button onClick={handleAutoPropagate} colorPalette="blue">
          <TbBadgesFilled />
          Auto Propagate
        </Button>
        <Dialog.Root
